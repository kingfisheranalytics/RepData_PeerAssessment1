...
if (!file.exists(fname)){
  stop("Error!  This script requires that the current working directory contain the \'activity.csv\' data file.  Please download the file, extract it, and copy it to this directory before trying again.")
}
...
The preprocessing step removed  `r omitted.cases` incomplete cases; the data frame containing only complete cases will be used for assignment Step 2 and Step 3.
...
**Results**: The mean number of steps taken per day was **`r stepsperday.mean.formatted`** and the median number of steps taken per day was **`r stepsperday.median.formatted`**.


 Results| Mean | Median
 --------------|---------|--------
 Steps Per Day |**`r stepsperday.mean.formatted`**|**`r stepsperday.median.formatted`**
<div style="height:96px; border-bottom:8px solid #2e3192;"></div>
...
**Results**: There are **`r interval.count`** intervals in the table, with the largest being **`r interval.largestinterval`**.  The maximum number of mean steps recorded for any interval was **`r format(interval.target$mean.steps, digits=2, big.mark=',', nsmall=2)`**.  This peak activity occurred at **Interval `r interval.target$interval`**.
